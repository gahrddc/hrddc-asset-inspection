...
function getPublicConfig(accessCode) {
  ...
  return {ok:true, round:publicRound_(round), roundState:roundState, statuses:statuses};
}
...
function findSourceRowFromIndex_(source, value) {
  const key = normalizeIndexKey_(value);
  const cache = CacheService.getScriptCache();
  let raw = cache.get('source_index_v2_' + indexBucket_(key));
  if (!raw) {
    buildSourceIndex_(source);
    raw = cache.get('source_index_v2_' + indexBucket_(key));
  }
  if (raw) {
    try {
      const row = Number(JSON.parse(raw)[key] || 0);
      if (row) return row;
    } catch (_) {}
  }
  const lastRow = source.getLastRow();
...
function saveInspection(payload) {
  ...
  try {
    ...
    const overview = db_().getSheetByName(ASSET_OVERVIEW_SHEET_);
    if (overview) setDynamicRoundStatusFormula_(overview);
    SpreadsheetApp.flush();
    log_('ตรวจครุภัณฑ์', round.id, String(payload.assetId), statusInfo.name + ' โดย ' + payload.inspector, String(payload.inspector));
    return {ok:true, message:'บันทึกผลเรียบร้อย', when:Utilities.formatDate(now,TZ,'dd/MM/yyyy HH:mm:ss')};
  } finally {
    lock.releaseLock();
  }
}
...
function updateRoundAdmin(form) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]) === id) {
      ...
      CacheService.getScriptCache().remove('active_round');
...
function getStatus_(code) {
  const rows = readObjects_(db_().getSheetByName('รายการสถานะ'));
  const found = rows.find(x => String(x['รหัส']) === String(code) && truthy_(x['ใช้งาน']));
  if (!found) throw new Error('สถานะไม่ถูกต้อง');
  return {name:String(found['ชื่อสถานะ']),noteRequired:truthy_(found['ต้องระบุหมายเหตุ'])};
}
...
function getAssetPhoto_(assetNo) {
  ...
  for (let i=rows.length-1;i>=0;i--) {
    if (String(rows[i][0]).trim() === key) return {assetNo:key,fileId:rows[i][3],url:rows[i][4],updatedAt:rows[i][5],updatedBy:rows[i][6]};
  }
  return null;
}
...
function adminJs_(){return `
  ...
  function startNotificationPolling(){
    ...
    notificationTimer=setInterval(()=>loadNotifications(false),30000);
  }
  function loadNotifications(initial){
    ...
    }).withFailureHandler(()=>{}).getAdminNotifications();
  }
